...
#if defined( USART0_ENABLED ) // CCD-bus
	#if defined( ATMEGA_USART ) || defined( ATMEGA_USART0 )
		static volatile uint8_t  UART_TxBuf[UART_TX0_BUFFER_SIZE];
		static volatile uint16_t UART_RxBuf[UART_RX0_BUFFER_SIZE]; // modified from uint8_t to uint16_t
...
#if defined( USART1_ENABLED ) // SCI-bus
	#if defined( ATMEGA_USART1 )
		static volatile uint8_t  UART1_TxBuf[UART_TX1_BUFFER_SIZE];
		static volatile uint16_t UART1_RxBuf[UART_RX1_BUFFER_SIZE]; // modified from uint8_t to uint16_t
...
#if defined( USART2_ENABLED ) // USB Packets
...
#if defined( USART3_ENABLED ) // Not in use by this library
...
**************************************************************************/
{
	...
	else
	{
        ...
        UART_RxBuf[tmphead] = lastRxError << 8 | data;
    }
...
/*************************************************************************
Function: uart0_peek_addr()
Purpose:  Returns the byte (character) of incoming UART data at the given
          location (addr) without removing it from the ring buffer.
Returns:  lower byte:  next byte in ring buffer
          higher byte: last receive error
Notes:    address is zero-based
**************************************************************************/
uint16_t uart0_peek_addr(uint16_t addr)
{
	uint16_t tmptail;
	uint16_t data;
	...
	tmptail = (UART_RxTail + 1 + addr) & UART_RX0_BUFFER_MASK;
...
**************************************************************************/
{
	...
	if ( tmphead == UART1_RxTail ) {
		...
		/* store received data in buffer */
		UART1_RxBuf[tmphead] = lastRxError << 8 | data;
	}
...
**************************************************************************/
{
	...
	if ( tmphead == UART2_RxTail ) {
		...
		/* store received data in buffer */
		UART2_RxBuf[tmphead] = lastRxError << 8 | data;
	}
...
/*************************************************************************
Function: uart2_available_rx()
Purpose:  Determine the number of bytes waiting in the receive buffer
Input:    None
Returns:  Integer number of bytes in the receive buffer
**************************************************************************/
uint16_t uart2_available_rx(void)
{
...
**************************************************************************/
uint16_t uart2_available_tx(void)
{
	return (UART_TX2_BUFFER_SIZE + UART2_TxHead - UART2_TxTail) & UART_TX2_BUFFER_MASK;
} /* uart2_available_tx */
